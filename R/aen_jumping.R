...
#' @export
aen_get_championship_data <- function(
  table,
  last_championship,
  engagement_deadline
) {
  table |>
    ...
    dplyr::rowwise() |>
    dplyr::mutate(
      data = list(aen_get_championship_infos(
        reiter_id,
        pferd_id,
        last_championship,
        engagement_deadline
...
#' @export
aen_get_championship_infos <- function(
  rider_id,
  horse_id,
  last_championship,
  engagement_deadline
  ...
  rider_id |>
    rsvps::get_fnch_rider_jumping_results() -> rider_results
  ...
  horse_results |>
    ...
    dplyr::mutate(
      height = readr::parse_number(kategorie_code, na = c("SP/CS"))
    ) |>
    tidyr::drop_na(height) -> horse_results_clean

  # Starts = RIDER starts
  rider_results |>
    dplyr::filter(datum > last_championship) |>
    dplyr::mutate(
      height = readr::parse_number(kategorie_code, na = c("SP/CS"))
    ) |>
    tidyr::drop_na(height) |>
    dplyr::summarise(par90 = sum(height > 90)) -> starts
  ...
  horse_results_clean |>
    ...
    dplyr::summarise(sop = sum(gwp, na.rm = TRUE)) |>
    dplyr::mutate(dplyr::across(
      dplyr::everything(),
      ~ tidyr::replace_na(.x, 0)
    )) -> sop
  ...
  return(c(
    list(mobile = rider_details$mobile, lic = rider_infos$lizenzen),
    starts |> as.list(),
